...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        #print 'noisyDistance: {}'.format(noisyDistance)
        #print 'emissionModel: {}'.format(emissionModel)
        #print 'pacmanPosition: {}'.format(pacmanPosition)
        if(not noisyDistance == None):
            for p in self.legalPositions:
                self.beliefs[p] = self.beliefs[p] * emissionModel[util.manhattanDistance(pacmanPosition, p)]
            self.beliefs.normalize()
        else:
            self.beliefs = util.Counter()
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos in newPosDist:
                newBeliefs[newPos] = newBeliefs[newPos] + self.beliefs[p] * newPosDist[newPos] #update new belief = sum with other new beliefs + old belief * new transition distribution where I came from
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState): #Q4
        """
    ...
    def observe(self, observation, gameState): #Q4
        ...
        if(not noisyDistance == None): #ghost is captured by pacman
            for particle in self.particleList:
                distanceOfParticleFromPacman = util.manhattanDistance(particle, pacmanPosition)
                newWeightForParticle = emissionModel[distanceOfParticleFromPacman]
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        for x in range(0, self.numParticles):
            newPossiblePositionDistForParticle = self.getPositionDistribution(self.setGhostPosition(gameState, self.particleList[x]))
            self.particleList[x] = util.sample(newPossiblePositionDistForParticle)

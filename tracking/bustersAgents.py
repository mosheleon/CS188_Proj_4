...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]] #list of all remaining (alive) ghosts
        "*** YOUR CODE HERE ***"
        #find the most likely position of each remaining (uncaptured) ghost
        maxDistributionForGhostAsPosition = []
        for ghost in livingGhostPositionDistributions:
            #print ghost
            #print ghost.argMax()
            maxDistributionForGhostAsPosition.append(ghost.argMax())
        
        minDistance = float("inf")
        returnPosition = None
        for position in maxDistributionForGhostAsPosition:
        ...
        for action in legal:
            ...
            if(self.distancer.getDistance(successorPosition, returnPosition) < minDistance ):
                bestAction = action
                minDistance = self.distancer.getDistance(successorPosition, returnPosition)
